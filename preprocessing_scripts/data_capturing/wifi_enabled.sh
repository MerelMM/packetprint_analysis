...
# capturing traffic, not explicitely turning the other apps off when starting capturing new app traffic (use wifi_enabled_2otherwise)
...
while IFS= read -r APP_PACKAGE <&3 || [ -n "$APP_PACKAGE" ]; do
    ...
    # 🛑 Force-stop the previously running foreground app
    ...
    if [[ -n "$PREV_APP" && "$PREV_APP" != "$APP_PACKAGE" ]]; then
    	echo "[*] Force-stopping previously running app: $PREV_APP" | tee -a "$LOGFILE"
    ...
    TOTAL_EVENTS_DONE=0
    # Kill processes of (other) apps running
